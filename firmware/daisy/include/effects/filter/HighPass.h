#pragma once
#include "Effect.h"
#include <cmath>
#include <cstring>

// Resonant high-pass for removing rumble or thinning a guitar signal.
// A normalized RBJ biquad gives a 12 dB/octave slope with controllable Q;
// coefficient normalization and a cutoff below Nyquist keep its poles stable.
class HighPass : public Effect {
public:
    void Init(float sample_rate) override {
        sample_rate_ = sample_rate > 1000.0f ? sample_rate : 48000.0f;
        cutoff_ = 120.0f;
        resonance_ = 0.2f;
        mix_ = 1.0f;
        x1_ = x2_ = y1_ = y2_ = 0.0f;
        UpdateCoefficients();
    }

    float Process(float in) override {
        const float wet = b0_ * in + b1_ * x1_ + b2_ * x2_ - a1_ * y1_ - a2_ * y2_;
        x2_ = x1_; x1_ = in;
        y2_ = y1_; y1_ = wet;
        return in * (1.0f - mix_) + wet * mix_;
    }

    const char* GetName() const override { return "highpass"; }
    EffectCategory GetCategory() const override { return EffectCategory::Filter; }

    void SetParam(const char* name, float value) override {
        if (strcmp(name, "cutoff") == 0) { cutoff_ = Clamp(value, 20.0f, MaxFrequency()); UpdateCoefficients(); }
        else if (strcmp(name, "resonance") == 0) { resonance_ = Clamp(value, 0.0f, 1.0f); UpdateCoefficients(); }
        else if (strcmp(name, "mix") == 0) mix_ = Clamp(value, 0.0f, 1.0f);
    }

    float GetParam(const char* name) override {
        if      (strcmp(name, "cutoff") == 0) return cutoff_;
        else if (strcmp(name, "resonance") == 0) return resonance_;
        else if (strcmp(name, "mix") == 0) return mix_;
        return 0.0f;
    }

    const char* GetParamList() const override { return "cutoff,resonance,mix"; }
    int GetParamCount() const override { return 3; }
    bool GetParamInfo(int index, EffectParamInfo& info) const override {
        switch (index) {
            case 0: info = {"cutoff", "Cutoff", "Hz", "float", "log", 20.0f, MaxFrequency(), 120.0f, 1.0f}; return true;
            case 1: info = {"resonance", "Resonance", "", "float", "linear", 0.0f, 1.0f, 0.2f, 0.01f}; return true;
            case 2: info = {"mix", "Mix", "", "float", "linear", 0.0f, 1.0f, 1.0f, 0.01f}; return true;
            default: return false;
        }
    }

private:
    float MaxFrequency() const { return sample_rate_ * 0.45f < 20000.0f ? sample_rate_ * 0.45f : 20000.0f; }
    void UpdateCoefficients() {
        const float omega = 2.0f * 3.14159265f * cutoff_ / sample_rate_;
        const float cosine = cosf(omega);
        const float q = 0.5f + resonance_ * 9.5f;
        const float alpha = sinf(omega) / (2.0f * q);
        const float inverse_a0 = 1.0f / (1.0f + alpha);
        b0_ = 0.5f * (1.0f + cosine) * inverse_a0;
        b1_ = -(1.0f + cosine) * inverse_a0;
        b2_ = b0_;
        a1_ = -2.0f * cosine * inverse_a0;
        a2_ = (1.0f - alpha) * inverse_a0;
    }

    float sample_rate_, cutoff_, resonance_, mix_;
    float b0_, b1_, b2_, a1_, a2_;
    float x1_, x2_, y1_, y2_;
};