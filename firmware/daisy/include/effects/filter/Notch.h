...
// Narrow notch filter for suppressing hum, feedback, or a harsh resonance.
// An RBJ biquad places zeros on the selected frequency and matched poles just
// inside the unit circle; resonance controls Q and therefore notch width.
class Notch : public Effect {
public:
    ...
    bool GetParamInfo(int index, EffectParamInfo& info) const override {
        switch (index) {
            case 0: info = {"frequency", "Frequency", "Hz", "float", "log", 20.0f, MaxFrequency(), 1000.0f, 1.0f}; return true;
            case 1: info = {"resonance", "Resonance", "", "float", "linear", 0.0f, 1.0f, 0.5f, 0.01f}; return true;
...
private:
    ...
    void UpdateCoefficients() {
        ...
        const float q = 0.5f + resonance_ * 19.5f;
