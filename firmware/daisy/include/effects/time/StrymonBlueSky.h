...
public:
    ...
    void Init(float sample_rate) override {
        ...
        // Initialize LFO caching variables
        lfo_subsample_counter_ = 0;
        cached_lfo_ = 0.0f;
        cached_lfo_quad_ = 1.0f;
    ...
    float Process(float in) override {
        ...
        // OPTIMIZATION 2: Downsample the LFO to calculate sinf only once every 16 samples
        if (++lfo_subsample_counter_ >= 16) {
            lfo_subsample_counter_ = 0;
            cached_lfo_ = sinf(lfo_phase_ * 6.28318531f);
            cached_lfo_quad_ = sinf(lfo_phase_ * 6.28318531f + 1.57079633f);
        }

        float y[4];
        for (int line = 0; line < 4; line++) {
            float polarity = (line & 1) ? cached_lfo_quad_ : cached_lfo_;
        ...
        for (int line = 0; line < 4; line++) {
            ...
            // OPTIMIZATION 3: Replaced heavy standard tanhf with FastTanh
            tank_[line][write_[line]] = FlushTiny(FastTanh(input * 0.92f));
        ...
        if (mode == 2) {
            ...
            // OPTIMIZATION 3: Replaced tanhf here as well
            tank_out = tank_out * 0.72f + FastTanh(spring_bp_ * 2.1f) * 0.28f;
        }
...
private:
    // OPTIMIZATION 3: Fast polynomial approximation of tanh(x)
    ...
    // LFO cache & subsampling state
    int lfo_subsample_counter_ = 0;
    float cached_lfo_ = 0.0f;
    float cached_lfo_quad_ = 1.0f;

    int pre_write_ = 0;
    int write_[4] = {};

    // OPTIMIZATION 1: Removed DSY_SDRAM_BSS so they allocate in fast SRAM
    static inline float pre_buffer_[MAX_PRE];
    static inline float tank_[4][MAX_LINE];
};